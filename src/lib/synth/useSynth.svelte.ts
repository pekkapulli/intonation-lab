...
import type { MelodyItem, SynthOptions, SynthVoice } from './types';
...
export function createSynth(options: SynthOptions = {}): SynthVoice {
	...
	let continuousVoice: { osc: OscillatorNode; gain: GainNode } | null = null;
	...
	function playFrequency(frequency: number) {
		...
		const { osc, gain } = continuousVoice;
		const previousFrequency = osc.frequency.value || frequency;
		osc.frequency.cancelScheduledValues(now);
		osc.frequency.setValueAtTime(previousFrequency, now);
		osc.frequency.linearRampToValueAtTime(frequency, now + 0.08);
		gain.gain.cancelScheduledValues(now);
		gain.gain.setValueAtTime(gain.gain.value || volume, now);
	...
	function setOptions(opts: Partial<SynthOptions>) {
		...
		if (continuousVoice && previousA4 !== a4 && lastActiveFrequency > 0) {
			...
			const osc = continuousVoice.osc;
			const gain = continuousVoice.gain;
			osc.frequency.cancelScheduledValues(now);
			osc.frequency.setValueAtTime(osc.frequency.value || lastActiveFrequency, now);
			osc.frequency.linearRampToValueAtTime(targetFrequency, now + 0.12);
			gain.gain.cancelScheduledValues(now);
			gain.gain.setValueAtTime(gain.gain.value || volume, now);
